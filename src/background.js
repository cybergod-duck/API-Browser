...
import { routeTask, getModelConfig, setMode, getMode, recordFailure, getTaskHistory, resetSession, TASK_PROFILES, loadUserKeys, getKeyStatus } from './model_router.js';
...
const MESSAGE_HANDLERS = {
  ...
  ROUTE_TASK: async (msg) => {
    try {
      const config = await routeTask(msg.taskType, msg.options || {});
  ...
  GET_ROUTER_INFO: () => ({
    ...
    models: {}, // live models come from GET_CATALOG / DISCOVER_MODELS (model_catalog.js)
    mode: getMode(),
  }),
  ...
  /** Get all scored models (advanced toggle) */
...
async function handleChatCompletion(provider, messages, options = {}) {
  ...
  if (provider) {
    ...
    const routed = await routeTask(taskType, options);
    config = getModelConfig(routed && routed.provider ? routed.provider : null);
  }
  if (!config) {
